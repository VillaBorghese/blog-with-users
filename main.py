...
# Initialize gravatar with flask app and default parameters
...
def register():
    ...
    if form.validate_on_submit():
        ...
        user = results.scalar()
        # Case 2: if the email already exist in the db:
        if user:
        ...
        # Adding the user to the db and committing the changes
        db.session.add(user)
        db.session.commit()
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and check_password_hash(user.password, password):
            ...
            flash("Email address doesn't exist, please provide a valid email")
        else:
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        if current_user.is_authenticated:
            ...
            flash("Please login or register, to comment a blog post")
            return redirect(url_for("login"))
